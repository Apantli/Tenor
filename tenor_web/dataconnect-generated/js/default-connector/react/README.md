...
You can use this generated SDK by importing from the package `@firebasegen/tenor-muse/react` as shown below. Both CommonJS and ESM imports are supported.
...
## ListLogs
You can execute the `ListLogs` Query using the following Query hook function, which is defined in [default-connector/react/index.d.ts](./index.d.ts):
```javascript
useListLogs(options?: useDataConnectQueryOptions<ListLogsData>): UseQueryResult<FlattenedQueryResult<ListLogsData, undefined>, FirebaseError>;
```
You can also pass in a `DataConnect` instance to the Query hook function.
```javascript
useListLogs(dc: DataConnect, options?: useDataConnectQueryOptions<ListLogsData>): UseQueryResult<FlattenedQueryResult<ListLogsData, undefined>, FirebaseError>;
```

### Variables
The `ListLogs` Query has no variables.
### Return Type
Recall that calling the `ListLogs` Query hook function returns a `UseQueryResult` object. This object holds the state of your Query, including whether the Query is loading, has completed, or has succeeded/failed, and any data returned by the Query, among other things.

To check the status of a Query, use the `UseQueryResult.status` field. You can also check for pending / success / error status using the `UseQueryResult.isPending`, `UseQueryResult.isSuccess`, and `UseQueryResult.isError` fields.

To access the data returned by a Query, use the `UseQueryResult.data` field. The data for the `ListLogs` Query is of type `ListLogsData`, which is defined in [default-connector/index.d.ts](../index.d.ts). It has the following fields:
```javascript
...
### Using `ListLogs`'s Query hook function

```javascript
import { getDataConnect, DataConnect } from 'firebase/data-connect';
import { connectorConfig } from '@firebasegen/tenor-muse';
import { useListLogs } from '@firebasegen/tenor-muse/react'

export default function ListLogsComponent() {

  // You don't have to do anything to "execute" the Query.
  // Call the Query hook function to get a `UseQueryResult` object which holds the state of your Query.
  const query = useListLogs();

  // You can also pass in a `DataConnect` instance to the Query hook function.
  const dataConnect = getDataConnect(connectorConfig);
  const query = useListLogs(dataConnect);
